...
4. 对于每条新闻，随机切换谁先介绍标题、谁做评论。不要总是同一个人先说。大约一半的新闻由{female_name}先介绍，另一半由{male_name}先介绍
5. 从第二条新闻开始，介绍标题的人必须先说一个简短的过渡词再引出标题，例如"接下来"、"下一条"、"然后是"、"再来看看"、"说到这个"等。过渡词要自然多样，不要每次都用同一个。第一条新闻不需要过渡词。
6. 总字数严格控制在 2800 字以内（约 7-8 分钟音频），这是硬性上限，宁可每个故事少说几句也不要超字数
...
<Person2>接下来，有个关于 Rust 重写 Linux 内核模块的讨论。</Person2>
<Person1>这个话题一直很火，社区的反应也很有意思。</Person1>
<Person1>再来看看，Google 发布了新的 AI 编程助手。</Person1>
<Person2>确实值得关注，不过我觉得还需要时间验证效果。</Person2>
"""
